...
import be.maximvdw.cmdBook.updater.Updater;
// Vault
...
public class cmdBook extends JavaPlugin {
	...
	public static int cb_buildversion = 152; // The craftbukkit version, this
												// app was build for
	...
	public void onEnable() {
		...
		int cb_version = Integer.parseInt(Bukkit.getBukkitVersion()
				.substring(0, Bukkit.getBukkitVersion().indexOf("-"))
				.replaceAll("\\.", ""));
		if (cb_version != cb_buildversion) {
			this.logger.severe(cmdFormat
					+ "cmdBook was not made for this craftbook version!");
		}
		// Show craftbook version
		this.logger.info(cmdFormat
				+ "Using craftbukkit: "
				+ Bukkit.getBukkitVersion().substring(0,
		...
		try {
			...
			this.logger
					.warning(cmdFormat
							+ "Unable to start Config saver! - This might result in an error!");
		}
		...
		try {
			...
			this.splitCmd = Configuration.config.getString("cmd_split"); // Get
																			// split
																			// character
			this.chestRadius = Configuration.config.getInt("chestRadius"); // Get
																			// chest
																			// Radius
		...
		if (Configuration.config.getBoolean("checkUpdates") == true
				|| Configuration.config.getBoolean("autoUpdate") == true) {
			this.logger.info(cmdFormat
					+ "Checking for updates on dev.bukkit.org ...");
			this.getServer().getScheduler()
					.runTaskLaterAsynchronously(this, new Runnable() {
						public void run() {
							new Updater(plugin, 44237,plugin.getFile(),
